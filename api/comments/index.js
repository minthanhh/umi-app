...
// src/.umi-production/api/comments/index.ts
var comments_exports = {};
__export(comments_exports, {
  default: () => comments_default2
});
...
async function comments_default(req, res) {
  switch (req.method) {
    case "GET":
      try {
        ...
        const [comments, total] = await Promise.all([
          prisma_default.comment.findMany({
            skip,
            take: limit,
            where,
          ...
          prisma_default.comment.count({ where })
        ]);
        res.status(200).json({
          data: comments,
          total,
    ...
    case "POST":
      try {
        ...
        if (!content || !taskId || !authorId) {
          return res.status(400).json({ error: "Content, taskId, and authorId are required" });
        }
        const comment = await prisma_default.comment.create({
          data: {
            content,
...
var comments_default2 = async (req, res) => {
  ...
  await comments_default(umiReq, umiRes);
};
