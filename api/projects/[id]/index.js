...
// src/.umi-production/api/projects/[id]/index.ts
var id_exports = {};
__export(id_exports, {
  default: () => id_default2
});
...
// src/api/projects/[id]/index.ts
async function id_default(req, res) {
  const { id } = req.params;
  ...
  switch (req.method) {
    ...
    case "PUT":
      try {
        ...
        const project = await prisma_default.project.update({
          where: { id: projectId },
          data: {
            ...name && { name },
    ...
    case "DELETE":
      try {
        await prisma_default.project.delete({
          where: { id: projectId }
        });
        res.status(204).end();
      } catch (error) {
...
var id_default2 = async (req, res) => {
  ...
  await id_default(umiReq, umiRes);
};
