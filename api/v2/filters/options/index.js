...
// src/.umi-production/api/v2/filters/options/index.ts
var options_exports = {};
...
// src/api/v2/filters/options/index.ts
function parseParentValue(value) {
...
async function handler(req, res) {
  ...
  const { ids, cursor, limit = "10", keyword, parentField, parentValue } = req.query;
  try {
    if (ids) {
      const idList = ids.split(",").map(Number).filter(Boolean);
      const data2 = await prisma_default.savedFilter.findMany({
        where: { id: { in: idList } }
      });
      return res.status(200).json({ data: data2, success: true });
    }
    const conditions = [];
    ...
    const [filters, total] = await Promise.all([
      prisma_default.savedFilter.findMany({
        where,
        take: take + 1,
        orderBy: { id: "asc" },
        ...cursor && { cursor: { id: Number(cursor) }, skip: 1 }
      }),
      prisma_default.savedFilter.count({ where })
    ]);
...
var options_default = async (req, res) => {
