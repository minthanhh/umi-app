...
// src/.umi-production/api/users/[id]/projects.ts
...
// src/api/users/[id]/projects.ts
async function projects_default(req, res) {
  if (req.method !== "GET") {
  ...
  try {
    ...
    const [projectMembers, total] = await Promise.all([
      prisma_default.projectMember.findMany({
        where: { userId },
        skip,
        take: limit,
        include: {
      ...
      prisma_default.projectMember.count({ where: { userId } })
    ]);
...
var projects_default2 = async (req, res) => {
  ...
  await projects_default(umiReq, umiRes);
};
