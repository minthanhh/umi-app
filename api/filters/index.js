...
// src/.umi-production/api/filters/index.ts
var filters_exports = {};
__export(filters_exports, {
  default: () => filters_default2
});
...
// src/api/filters/index.ts
async function filters_default(req, res) {
  switch (req.method) {
    case "GET":
      try {
        ...
        const filters = await prisma_default.savedFilter.findMany({
          where,
    ...
    case "POST":
      try {
        ...
        if (isDefault) {
          await prisma_default.savedFilter.updateMany({
            where: { page, isDefault: true },
            data: { isDefault: false }
          });
        }
        const filter = await prisma_default.savedFilter.upsert({
    ...
    case "DELETE":
      try {
        ...
        await prisma_default.savedFilter.delete({
          where: { name_page: { name, page } }
        });
...
var filters_default2 = async (req, res) => {
  ...
  await filters_default(umiReq, umiRes);
};
