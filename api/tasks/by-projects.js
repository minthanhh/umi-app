...
// src/.umi-production/api/tasks/by-projects.ts
var by_projects_exports = {};
__export(by_projects_exports, {
  default: () => by_projects_default2
});
module.exports = __toCommonJS(by_projects_exports);
...
// src/api/tasks/by-projects.ts
async function by_projects_default(req, res) {
  if (req.method !== "POST") {
  ...
  try {
    ...
    if (ids.length > 0) {
      ...
      whereClause = {
        projectId: { in: projectIds.map((id) => Number(id)) }
      };
    }
    ...
    const [tasks, total] = await Promise.all([
      ...
      prisma_default.task.count({ where: whereClause })
    ]);
    res.status(200).json({
      data: tasks,
      total,
...
var by_projects_default2 = async (req, res) => {
  ...
  await by_projects_default(umiReq, umiRes);
};
